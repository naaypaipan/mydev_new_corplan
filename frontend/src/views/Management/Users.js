...
function Users({ title, subtitle }) {
  ...
  useEffect(() => {
    dispatch(actions.userAll({ name, page, size }));
  ...
  const onSubmit = async (data) => {
    if (data.password === data.confirmPassword) {
      try {
        ...
        alert('สำเร็จ');
        handleClose();
        await dispatch(actions.userAll({ name, page, size }));
      } catch (error) {
        console.error(error);
      }
    } else {
  ...
  const handleDeleteUser = async (id) => {
    ...
    if (confirm) {
      try {
        await dispatch(actions.userDelete(id));
        await dispatch(actions.userAll({ name, page, size }));
      } catch (error) {
        console.error(error);
      }
          ...
          <Table sx={{ minWidth: 650 }}>
            <TableHead>
              <TableRow>
                ...
                <TableCell>
                  <div className="font-bold">ชื่อผู้ใช้</div>
                </TableCell>
                <TableCell>
                    ...
                    <TableCell>
                      <div className="flex flex-row flex-wrap gap-1">
                        <Button
                          ...
                          color={'warning'}
                          size={'small'}
                          onClick={() => handlePushToEditUser(row.id)}
                        >
                          แก้ไข
                        </Button>
                        <Button
