...
function ExpensesList({ title, subtitle }) {
  ...
  const totalAmount =
    filteredExpenses?.reduce(
      (sum, expense) => sum + (expense?.net_price || 0),
      0,
    ) || 0;
  const pendingCount =
  ...
  const pendingAmount =
    filteredExpenses
      ...
      .reduce((sum, expense) => sum + (expense?.net || 0), 0) || 0;
  const approvedCount =
        ...
        <Box sx={style}>
          ...
          {detailData && (
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6}>
                <Typography variant="subtitle2" color="textSecondary">ยอดรวม (บาท)</Typography>
                <Typography variant="h6" fontWeight="bold" color="error">
                  ฿{detailData?.net?.toFixed(2)?.replace(/\d(?=(\d{3})+\.)/g, '$&,')}
                </Typography>
              </Grid>
              {detailData?.includes_vat && (
                <Grid item xs={12} sm={6}>
                  <Typography variant="subtitle2" color="textSecondary">VAT 7% (รวมในยอดรวม)</Typography>
                  <Typography variant="body1" fontWeight="bold" color="info.main">
                    ฿{detailData?.vat_amount?.toFixed(2)?.replace(/\d(?=(\d{3})+\.)/g, '$&,')}
                  </Typography>
                </Grid>
              )}
              {detailData?.withholding_tax ? (
                <Grid item xs={12} sm={6}>
                  <Typography variant="subtitle2" color="textSecondary">หัก ณ ที่จ่าย 3%</Typography>
                  <Typography variant="body1" fontWeight="bold" color="warning.main">
                    - ฿{detailData?.withholding_tax?.toFixed(2)?.replace(/\d(?=(\d{3})+\.)/g, '$&,')}
                  </Typography>
                </Grid>
              ) : null}
              <Grid item xs={12} sm={6}>
                ...
                <Typography variant="h6" fontWeight="bold" color="success.main">
                  ฿{(detailData?.net_net ?? ((detailData?.net || 0) - (detailData?.withholding_tax || 0))).toFixed(2)?.replace(/\d(?=(\d{3})+\.)/g, '$&,')}
                </Typography>
              </Grid>
              <Grid item xs={12}>
              ...
              {/* {detailData?.budget && (
                <Grid item xs={12}>
                  ...
                  <LinearProgress 
                    variant="determinate" 
                    value={Math.min(((detailData?.budget?.total_expenses || 0) + detailData?.net) / (detailData?.budget?.cost || 1) * 100, 100) || 0} 
                    color={((detailData?.budget?.total_expenses || 0) + detailData?.net) / (detailData?.budget?.cost || 1) * 100 > 90 ? 'error' : 'primary'}
