...
export default function EditExpenses({ title, subtitle }) {
  ...
  useEffect(() => {
    ...
    setValue('project', expenses?.project?.id || expenses?.project?._id);
    setValue('expenses_type', expenses?.expenses_type || 'REFUND');
    setDate(expenses?.date);
    setCustomerSelect(expenses?.customer);
    // Initialize project & budget from loaded data
    if (expenses?.project) {
      setProjectSelect(expenses?.project);
    }
    if (expenses?.budget) {
      setBudgetSelect(expenses?.budget);
    }
    return () => {};
  }, [expenses]);
